...
지금까지 우리가 다 혼자서 한거임. 집에서 서비스를 운영하는 것은 상당히 제한적이고 불편한점이있음.

그러한 불편함을 해결해주는 서비스가 있음.. 그것은 바로 호스팅.. HOSTING은 클라이언트 쪽이 아니고 정보를 제공하는 사업자. 서버쪽에서 사용되는 컴퓨터 혹은 소프트웨어를 제공하는 사업자를 호스팅이라고 부름.

호스팅에는 어떤 환경을 갖추고 비즈니스를 하냐면 IDC(internet data center) 가있음 전기가 차단되면 자가발전을 하는.. 인터넷도 빵빵. 서버가 동작하기엔 최적. 보안과 네트워크..

이런 인프라를 갖추고 사용자에게 웹 애플리케이션을 운영하고자 하는 사람들에게 서버의 운영을 대행 (호스팅 사업) 대표적으로 웹호스팅과 서버호스팅이 있음.

**웹호스팅(Web Hosting)**

웹 애플리케이션을 운영을 위한 모든 것을 제공 웹서버(Apache, NGINX), 미들웨어(PHP, JAVA, Python, Ruby), 데이터베이스(MYSQL, ORACLE, SQL Server) + 서버

장점 = 웹 애플리케이션을 업로드만 하면 됩니다.
단점 = 웹 애플리케이션을 위한 용도로만 사용.

일반적으로 웹 호스팅은 한대의 컴퓨터에 여러명의 사용자가 공유하기 때문에 한명이 대박이 나면 자신의 사이트가 느려짐.

하지만 가장 사용하기 쉽고 비용도 저렴하기 때문에.. 제일 먼저 고려해 볼만한 호스팅

**서버호스팅(Server Hosting)**

컴퓨터를 제공. 서버 컴퓨터를 제공. 서버 컴퓨터에는 웹서버, 미들웨어, 데이터베이스 하나도 설치 되있지 않고 필요한 것들을 직접 설치.
...
'현시점'에서는 대답하기 어려운 질문입니다. 각자의 장단이 존재하기 때문입니다. 강력함에는 비용이 따릅니다. 여러가지 비용이 있겠지만 학습비용을 생각해보면 좋을 것 같군요. 동네슈퍼와 대형마트를 비교해 볼까요? 대형마트는 가격도 저렴하고, 물건도 많습니다. 하지만 소비자는 대형마트의 체계를 익히고, 그 체계에 자신을 맞춰야 합니다. 주차방법, 계산방법, 반품절차, 포장방법, 캐비넷 사용법 등을 익히고 따라야 합니다. 이러한 것은 대형마트를 이용하면서 치뤄야 하는 비용입니다. 그럼에도 불구하고 대형마트가 장점이 있다면 이용할 것이고, 그렇지 않다면 슈퍼나 편의점을 이용하겠죠. 물론 이것은 정확하지 않은 비유일 뿐입니다. '현시점에서' 필자가 추천하는 방법은 가장 저렴한 웹호스팅을 선택해서 빠르게 서비스를 동작시키는 것입니다. 그리고 장기적으로는 비용 효율적인 클라우드를 익혀서 비즈니스의 규모가 커짐에 따라서 클라우드로 이동하는 것입니다. 

## 성장

성장통을 어떻게 극복할것인지.. 좀 더 규모 있는 애플리케이션 Scalability.

씨앗을 생각해보자. 씨앗은 어떤 성체가 될지 지금 당장 예측할수 없음. 씨앗은 생명이 필요한 최소한의 형태(생명의 본질)

씨앗이 싹이 나고 잎이 되고 나무가 되는 과정을 살펴봄.

HTML -> PHP,MYSQL 의 과정을 살펴봄.. 복잡도를 감수하면서 얻게된 효과들을 얘기하면서 여기까지 온것.

지금까지 우리가 살펴본 애플리케이션은 하나의 컴퓨터 위에서 동작했던 것. 이 시스템이 거대해지면서 규모가 생기면서 하나의 컴퓨터로 처리하는건 한계가 온다.

여러대의 컴퓨터를 통해서 이것을 해결 해야된다.

규모의 두가지 전략
* Scale UP(한대의 컴퓨터가 강력해지는 것) 가장 간단하고 쉬운 방법
* Scale OUT(여러개의 컴퓨터가 협력해서 동작하는 것)

인프라와 컴퓨터를 혼용해서 사용하겠다.

서버 컴퓨터에는 웹서버, PHP, MYSQL 세개가 같이 동작하게 되는데. 규모가 커지면서 점점 한계가 생김. 우리의 웹 애플리케이션이 점점 느려지고, 사람들이 많이 접속하면 죽게됨.

우리는 기회를 맞이 했지만 그 기회가 우리 서비스가 무너지는 위기이기도 하다. 이러한 위기에서 우리가 어떻게 살아 남을까? 이것을 어떻게 극복 하는가를 얘기할 것이다.

웹서버 - PHP - mysql 

여러분들은 어떠한 불안감에 휩싸이게됨.(서비스가 점점 느려지고 죽음) 컴퓨터를 열어보면 cpu 점유율이 100퍼에 육박하고 메모리가 여유가 없는 상태가 계속 됨.

여러분들의 비즈니스의 클라이막스와 동시에 위기.. 우리가 취할 수 있는 전략 여러대의 컴퓨터를 이용해서 작업. 웹서버, php, mysql 중에 가장 많은 일을 하는것을 분리 하는 작업.

일반적으로 데이터베이스가 가장 부담 스러운 작업을함. 데이터가 축적되면서 점점 부담스럽게됨. 새로운 컴퓨터(COMPUTER 2)를 구매하고 거기에 MySQL을 설치하고

php에서 mysqli_connect("COMPUTER 2", "root", "11111") 이런식으로 mysql 에 접속함.. 이러한 작업으로 우리는 두대의 컴퓨터로 우리의 웹 애플리케이션이 동작함.

스케일 아웃을 하면 네트워크를 통해서 작업을 하기 때문에 한대의 컴퓨터에서 작업하는 것보단 느리지만 이렇게 하는 것이 더 이익이 크기 때문에 컴퓨터를 분산하게 됨.

그렇게 조금 빨라졌지만 시간이 지남에 따라 다시 느려짐. 우리가 선택할 수 있는 전략은 scale out이나 scale up을 하는것.. 

computer3에 php를 설치.. computer1(웹서버), computer3(php)(애플리케이션 서버), computer2(MySQL)(데이터베이스 서버) 설치.

하지만 이것보다 더 성장해버리면... 스케일 업을 해보겠지만 그것도 한계가 생기면 스케일 아웃.. 컴퓨터를 한대 더 구매허서 computer4 기존에 있었던 컴퓨터는

MySQLMaster(computer2) 를 부여 그리고 MySQLSlave(computer4) mysql Slave 가 mysqlMaster의 값을 복제 하고.. php에서는 MySQL Master에는 쓰기작업

그리고 MySQLSlave 에서는 읽기 작업을 함..(상당히 복잡한 작업) 

이것을 코드로 살펴보면 이런 모습이 됨.

<pre>
$master = mysqli_connect("computer2", "root", "11111");
$slave = mysqli_connect("computer4", "root", "11111");
...
데이터베이스는 한대의 컴퓨터에서 쓰기를 전담하고, 다른 컴퓨터는 쓰기를 전담하는 컴퓨터의 내용을 복제하는것을 통해 데이터가 중복해서 쓰기 작업을 하는걸 방지한다.

쓰기는 마스터에. 읽기는 슬레이브에서.. 이렇게 해서 4대의 컴퓨터에 애플리케이션이 동작.. 부하가 좀더 줄어듬..

하지만 이것도 사용자가 많아지면 한계에 도달함..

또 한대의 컴퓨터를 구매하고.. computer5 설치한 데이터베이스에 MySQL Slave2 라는 이름을 붙임..

데이터를 수정하면 Slave1이 마스터의 정보를 카피 Slave2도 마스터의 정보를 카피.. 그 상태에서 읽기 관련된 작업이 실행되면..

랜덤하게 슬레이브 1이나 슬레이브 2의 데이터를 가져옴. 실제로 가장 빈번한 작업이 일어나는 작업이 읽기고 가장 부담 스러움.

이렇게 체계가 다져지면 슬레이브는 1,2,3.. 이런식으로 늘릴수있음.. 하지만 우리의 걱정거리는 master 컴퓨터에 초점이 맞춰짐..

결국엔 master도 한계가 일어남.. 쉽게 말해 페이스북같은 서비스는 10억명의 사용자가 사용하기 때문에.. 이문제는 샤딩으로 해결함 (쪼갠다)

MySql Master1(Computer2) 에서는 (1~1000번까지의 사용자의 정보를 저장) MySql Master2(Computer6)에는 (2000~3000번까지의 사용자 정보를 저장)

샤딩은 쉽게 접근할수 없는 방법임.. 하지만 결국엔 샤딩이 필요한 순간이 오게됨.. 샤딩의 단계가지 오게 되면 이걸 잘할수있는 전문가를 영입할 수 있는 단계가 됨..

이제 데이터베이스는 신경 쓸 필요가 없음..

웹서버나 php에 신경 써보자.

웹서버나 php는 어떻게 스케일 아웃을 할까? 데이터베이스보다 훨씬 더 쉽다. 그냥 논리적인 일을 처리하기 때문에.

php 를 스케일 아웃할때 새로운 컴퓨터(computer7)를 구입하고. 웹서버가 computer3 와 computer7에 각각 랜덤으로 보내면 됨.

데이터베이스의 분산 처리를 하는 주체는 php였음. php가 일종의 교통 경찰과 같은 역할을 해서 부하를 분산해 줬음. 두대의 php는 웹서버에 의해서 분산이 됬음.

웹서버 역시. 스케일 아웃을 하면 각각의 웹서버에 부하를 분산할수 있는 그 누군가가 누구냐???

웹서버의 부하 분산은 어떻게 가능하게 되나??

웹서버를 스케일 아웃 하는 방법은 여러가지 있음

주요한 두가지 방법

클라이언트를 자세히 들여다 볼 필요가 있음. 사용자가 우리의 컴퓨터에 접속할때... dns 서버가 ip주소를 가르켜주고 그 ip주소를 통해 접속을 하게 된다.

결국엔 웹서버의 컴퓨터에 접속을 할수가 있어야 되는데. 운영체제가 dns 서버에게 computer1의 ip를 가르켜줄거고 그 computer1에 접속하게 됨.

이러한 점에 착안해서 라운드 로빙이라는 방식을 사용할수 있음 computer8을 구입하고 웹서버를 설치함. dns 서버가 computer8번과 computer1번의 주소를 랜덤으로 가르켜줌..

접속하는 사람마다 서로다른 ip를 가르켜줘서 접속하는 방식이 라운드 로빙.

로드 밸런서를 사용(Load Balancer)

부하를 분산하는 소프트웨어 장치.

사용자는 로드 밸런서의 ip의 모든 사용자가 접속을 하게 되고. 로드 밸런서는 대량의 정보를 처리할수있는 어떤 시스템이고.. 상황에 따라 computer1 번과 computer8번에 보내게 됨.

로드 밸런서를 사용할 때 장점은. 각각의 웹서버에 컴퓨터를 감시하고 한가하거나 바쁜거에 따라 트래픽을 보냄. 그리고 죽은 서버에는 트래픽을 보내지 않음. 좀더 지능적인 부하 분산 가능

이것도 중요하지만 최근에 대두되는 트랜드는 클라우드 컴퓨팅이라고 하는 것이고 핵심은 가상화와 종량제. 가상화를 통해 이룰 수 있는 효과는 필요할때 자신이 필요한 컴퓨터를 빠르게 만들수 있다.

가상화 클라우드 컴퓨팅 쪽은 기본적으로 로드밸런싱을 제공하고 그런 클라우드 컴퓨팅에서 임대한 컴퓨터를 연결함. 이 로드 밸런싱에 연결되있는 컴퓨터의 상태를 체크하고 그 연결되있는 전체의 컴퓨터 점유율의 평균이 80퍼 센트 이상이 상태가

10분이 지나면(예를들어) 컴퓨터를 새로 만들어서 새로 만든 컴퓨터에 부하를 분산하고 여전히 80퍼 이상이면 또 자동으로 컴퓨터를 만들어줌.. 그리고 그러다가 cpu점유율이 20퍼센트 이하로 떨어지면 컴퓨터를 삭제해줌. 이러한 기능이 클라우드 컴퓨터가 가지고 있는 중요한 기능.

이러한 과정은 상당히 복잡한 과정. 편의점이나 동네 슈퍼, 대형마트 등 본질적으로 유통을 한다는 점은 같지만 대형마트는 장점만 있는 건 아님. 대형마트에서 쇼핑을 하려면 알아야 될것이 많다.(오만가지 방법들)

클라우드 컴퓨팅도 비슷함. 강력하지만 알야야 될것이 많다.(노력이 필요)

이렇게 되서 작은 씨앗의 html 어떻게 나무가 될까.. 라는 서버가 어떻게 복잡한 인프라가 될까를 살펴봄.
